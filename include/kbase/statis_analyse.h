...
namespace crx
{
    ...
    {
        unsigned int user;		//从系统启动开始累计到当前时刻，处于用户态的运行时间，不包含nice值为负的进程。
        unsigned int nice;		//从系统启动开始累计到当前时刻，nice值为负的进程所占用的CPU时间
        unsigned int system;	//从系统启动开始累计到当前时刻，处于核心态的运行时间
        unsigned int idle;			//从系统启动开始累计到当前时刻，除IO等待时间以外的其它等待时间
    };

    struct process_mem_occupy_t
    {
        unsigned int vm_peak;		//进程所占虚拟内存的峰值
        unsigned int vm_size;		//进程所占用的虚拟内存的值
        unsigned int vm_hwm;		//进程所占物理内存的峰值
        unsigned int vm_rss;		//进程所占用的实际物理内存
    };

    struct sys_mem_dist_t
    {
        unsigned int mem_total;		//系统的内存总量
    };
