...
namespace crx
{
    ...
    bool console_impl::preprocess(int argc, char *argv[])
    {
        ...
        epoll_thread_impl *eth_impl = static_cast<epoll_thread_impl*>(m_c->m_obj);
        eth_impl->m_cover = true;             //覆盖当前线程的方式运行
        ...
        if (argc >= 2) {		//当前服务以带参模式运行，检测最后一个参数是否为{"-start", "-stop", "-restart"}之一
            ...
            if (!strcmp("-restart", argv[argc-1])) {				//重启服务
                ...
                eth_impl->m_event = m_c->get_event(eth_impl->handle_request, eth_impl);
                execute_cmd({"-start"});
            }
        }
     ...
     */
    void console_impl::wait_shell_input(int wr_fifo)
    {
        epoll_thread_impl *eth_impl = static_cast<epoll_thread_impl*>(m_c->m_obj);
        eth_impl->m_cover = false;             //不覆盖当前线程的方式运行
        m_c->start();

        bool go_done = true, write_pipe = false;
        while (go_done) {
            if (!write_pipe) {
                printf("%s:\\>", g_server_name.c_str());
                fflush(stdout);
            }

            std::string input(1024, 0);
            int ret = read(STDIN_FILENO, &input[0], input.size()-1);
            if (-1 == ret || 0 == ret)
                break;
            ...
            auto str_vec = crx::split(input, " ");		//命令行输入的一系列参数都是以空格分隔
            if (!str_vec.empty()) {
                if ("q" == input)
                    go_done = false;

                if (-1 == wr_fifo) {		//wr_fifo等于-1表示当前程序是以普通进程方式运行的
                    if (!execute_cmd(str_vec))		//若需要执行的命令并未通过add_cmd接口添加时，通知该命令未知
                    ...
                    if ("h" == input || "q" == input) {     //输入为"h"(帮助)或者"q"(退出)时直接执行相应命令
                        ...
                        write(wr_fifo, input.data(), input.size());
                        write_pipe = true;
                    }
                }
            }
        ...
        if (eth_impl->m_thread.joinable())
            eth_impl->join_thread();
    }

    //当前程序以daemon进程在后台运行时，该进程通过管道接收命令行的输入参数
    void console_impl::wait_pipe_input(int stdout_bk)
    {
        //以非阻塞方式创建管道时，被动方首先打开非阻塞的读管道并在epoll上监听之
        //open函数带O_NONBLOCK标志调用时只表明不阻塞open函数，而并不表示对rd_fifo的读写同样也是非阻塞的
        int rd_fifo = open(m_pipe_name[0].c_str(), O_RDONLY | O_NONBLOCK), wr_fifo = -1;
        crx::setnonblocking(rd_fifo);		//因此调用setnonblocking将该文件描述符设置为非阻塞的
        ...
        eth_event *ev = new eth_event;
        ev->fd = rd_fifo;
        ev->eth_impl = (epoll_thread_impl*)m_c->m_obj;
        ev->f = [&](eth_event *ev) {
            //当读管道可读时，说明有其他进程的读写管道已经创建，并且写管道已经与本进程的读管道完成连接，
            //此时在本进程中创建写管道并与其他进程的读管道完成连接
            if (!conn) {
                wr_fifo = open(m_pipe_name[1].c_str(), O_WRONLY);
                //将写管道重定向至标准输出文件描述符，shell进程通过管道接收后台进程的一系列运行时输出信息
                dup2(wr_fifo, STDOUT_FILENO);
                conn = true;
            }

            std::string cmd_buf;
            int sts = epoll_thread_impl::async_read(ev->fd, cmd_buf);
    ...
    void console_impl::connect_service(bool stop_service)
    {
        ...
        ev->f = [&](eth_event *args) {
            std::string output;
            int sts = epoll_thread_impl::async_read(args->fd, output);
            if (!output.empty()) {
                printf("%s", output.c_str());		//打印后台daemon进程输出的运行时信息
                //当前shell既不是用来停止后台服务，也未出现管道异常，则打印命令行提示符
                if (!stop_service && !excep) {
                    printf("%s:\\>", g_server_name.c_str());
                    fflush(stdout);
                }
            }
            ...
            if (sts <= 0) {		//对端关闭或异常
                ...
                args->eth_impl->remove_event(args);
                args->eth_impl->join_thread();
            }
        };
        ev->args = ev;
        ev->eth_impl->add_event(ev);
    ...
    {
        epoll_thread_impl *eth_impl = static_cast<epoll_thread_impl*>(c->m_obj);
        console_impl *con_impl = static_cast<console_impl*>(eth_impl->m_obj);
    ...
    console::console()
    {
        ...
        epoll_impl->m_obj = new console_impl(this);
    }

    console::~console()
    {
        epoll_thread_impl *epoll_impl = static_cast<epoll_thread_impl*>(m_obj);
        delete (console_impl*)epoll_impl->m_obj;
    }
    ...
    void console::add_cmd(const char *cmd, std::function<void(std::vector<std::string>&, console*)> f,
                          const char *comment)
    {
        epoll_thread_impl *eth_impl = static_cast<epoll_thread_impl*>(m_obj);
        console_impl *con_impl = static_cast<console_impl*>(eth_impl->m_obj);
    ...
    int console::run(int argc, char *argv[])
    {
        ...
        if (!con_impl->m_is_service) {			//前台运行时在控制台输入接收命令
            con_impl->wait_shell_input(-1);
        } else {			//后台运行时从管道文件中接收命令
            int stdout_bk = dup(STDOUT_FILENO);
            con_impl->wait_pipe_input(stdout_bk);
            close(stdout_bk);
        }
